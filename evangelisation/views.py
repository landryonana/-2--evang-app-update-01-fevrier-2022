...
def evangelisation_app_index(request):
    context = dict()
    evang_first = None
    evangs = None
    annees = list()
    ...
    if ('liste-evang' or 'annee-evang') in request.GET:
        id = int(request.GET['liste-evang'])
        annee = int(request.GET['annee-evang'])
        evangs = evangs.filter(day__year=annee)
        evang_first = Evangelisation.objects.get(id=id)
    else:
